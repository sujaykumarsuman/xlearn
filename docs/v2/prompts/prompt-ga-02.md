...
> **One self-contained prompt = one sprint = one session.** Paste into a fresh coding session at the repo root **on GA day, with the owner present** for the whole sitting (~30 min of his time).
> **Plan:** [`../sprints/sprint-ga-02.md`](../sprints/sprint-ga-02.md)   ·   **Milestone:** GA (part 2 of 2)   ·   **Prereqs:** [ga-01](../sprints/sprint-ga-01.md) (GA PR merged with the owner's approval; `v2.0.0-rc.N` rehearsed; no active learner)
...
- ga-01's record in [`../status.md`](../status.md):
  ...
  - the owner's gc-visibility choice.
- **Code and infra:**
...
- **5.** `host-verify --cluster` + the owner's **manual snapshot**;
...
- [ ] **ga-01 ✅:**
  ...
  - the GA PR # was merged with the owner's approval;
...
- [ ] **The owner is present** with hPanel access, and has chosen his go-concurrency public visibility (AB22).
- [ ] **Parallel sessions:**
...
2. **[X] Pre-flip check** (plan task 2; ADR-0034 §1.4 step 3). Run `hack/ga-preflip-check.sh --major 2 --cluster` at the tag commit. **All PASS:**
   ...
   Save the table for the PR. **If a stray exists**, stop and bring the owner the two ADR branches:
   - **delete it:** `git push origin :refs/tags/<tag>` and `gh api -X DELETE …/packages/container/xlearn-<svc>/versions/<id>`. **Irreversible:** the owner runs or explicitly approves each deletion;
   - **cut GA above it:** tag a version above the stray, **before widening**. ADR-0034 §1.4 steps 4 and 6 swap (this prompt's steps 3 and 6), so the order is check → `host-verify` → snapshot → tag → widen → verify. Delete the stray **git** tag anyway (only a GHCR version can resist deletion), so `deploy.yml`'s no-contract step still sees the chosen tag as the major's first stable release; the step 1 lint at the chosen commit is the second guard. Record the deviation.
3. **[I] Widen** (plan task 3; step 4). In `../infra`, from a fresh `main`, branch `chore/xlearn-fleet-ranges-v2-ga`:
   ...
   - **Re-run step 2 if more than ~10 min have passed**, then squash-merge with the owner present.
...
5. **[O] Snapshot** (plan task 5, `ev-ga`):
   - The owner reads the **last weekly image date**. If it is more than 7 days old, stop.
   - The owner takes the **manual snapshot** (one at a time, ~1-day life) and gives you the time or id.
   - Go straight on to the tag in this sitting.
...
7. **[X] Verify + GA smoke** (plan task 7; step 7).
   ...
   - **The GA smoke:**
     - owner: packed DSA Run → Submit → `auto · checked`; an AI suggestion or the allowance meter; go-concurrency without the `preview` badge;
   ...
   - **Optional, only if the owner asks:** a tester → `set-role learner`, check Run/Submit and gc as a non-cohort account, then `set-role tester` back within the sitting. Record it with counts and roles only.
   - Post-rollout: `host-verify --cluster --with-runner --nats-stage=n4` green.
   - On any failure, go to the plan's **Rollback**: R-a per capability; **R-b** (narrow the 8 fleet policies back to `<2.0.0`) is the GA rollback; R-c = `v2.0.1`; R-d only within the snapshot's day, in the §4.2 order.
...
9. **[X] Record, end the freeze, land and sync** (plan task 9).
   ...
   - Land a docs-only PR → squash-merge.
   - Run `git checkout main && git pull` in **both** `xlearn` and `../infra`.

## Constraints

- **The ADR-0034 §1.4 order is law:** pre-flip → widen (merged **before** the tag) → `host-verify` + snapshot → tag → verify. The only exceptions are the ADR's own "stray can't be deleted" branch, recorded as a deviation, and undoing an R-b (tag `v2.0.1` first, then re-widen): "merge first" holds only while no stable 2.x exists.
...
- **Snapshot:** only once the host has settled; manual, right before the tag, in the same sitting (1-day life). No off-node `pg_dump` (D12).
...
- `host-verify --cluster` green; the owner's manual snapshot (time recorded).
...
- [ ] `host-verify --cluster --with-runner --nats-stage=n4` green on a settled host; weekly image ≤ 7 days; the manual snapshot taken right before the tag and recorded.
...
Ship per AGENT.md land-and-sync with **this sprint's release action: tag `v2.0.0`**. Before it, and in this order:
1. the pre-flip check;
2. **one infra PR** (the 8-policy superset widening, merged **before** the tag, never folded into it);
3. `host-verify --cluster` green;
4. the owner's manual snapshot.

After the verify, merge the docs PR, then run `git checkout main && git pull` in **both** `xlearn` and `../infra`. If a clean peer worktree holds `main`, use `git -C <worktree> merge --ff-only origin/main` instead.

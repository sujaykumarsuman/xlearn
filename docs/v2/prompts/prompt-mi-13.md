...
- [ ] `ssh vps 'bash /root/host-verify.sh --cluster'` (or `ssh vps 'bash -s -- --cluster' < ../infra/hack/host-verify.sh`): memory sum ≥ the coach resize's cost + 0.05 GiB inside the rule — cost = 2 × (S6 M8 memory limit − 128 Mi), so **≥ 0.3 GiB** at the default 256 Mi (record the number)
...
- **GitOps only:** never `kubectl apply`/`edit`/`debug`/`run` by hand; `ssh vps` reads and `host-verify.sh` only; `kubectl exec` only for the admin CLIs (`coach admin`). Mirror `../infra` conventions; one Middleware per namespace (no cross-namespace refs); no entrypoint-level header middleware.
...
- [ ] `POST /xlearn/api/v1/interviews/{id}/segments` is live on prod: cohort-only, 20 KiB cap (typed 413), **20 s** budget (typed 504), no retry, SDP never logged; the owner's signed-in fetch gets coach's 404 and coach's access log shows the request; signed out → 401; the prod timeout audit finds nothing on the path below 20 s
...
Ship at session end per AGENT.md land-and-sync with **this sprint's release action — infra PR(s) first, then tag the `v2.0.x` patch**: merge the `../infra` PRs once CI is green and let Flux reconcile them, merge the xlearn PR, tag `v2.0.N` after the release checklist (no live interviews), let Flux deploy and verify live, then `git checkout main && git pull` in xlearn and `../infra`.

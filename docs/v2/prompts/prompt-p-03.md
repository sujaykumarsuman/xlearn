...
- **This sprint closes P:** the quiz end to end (AB14), the race verdict view (AB15), `packlint`/content CI for the pilot, **tag v1.15.0**, then the **evalpack minor** with the go-race gates, then the owner's graded run on prod.
...
- [ ] **AB14 F11 resolved:** the owner's answer to the DS-P-01 freeze PR's open question (arena answers behind a D17 spoiler vs t4 §4.3 / ADR-0029 §2). Record it in the Decisions log. **No answer: stop and ask.**
...
- [ ] **AB14/AB15 frozen** (the DS-P-01 PR merged; status.md artboard rows "frozen (PR #, date)").
...
1. **[X] Key grading for the gc quizzes.**
   ...
   - **Mode selector (if the frozen schema lacks one):** add optional `choice.mode` (`single|multi|order`, default `single`) and `blank.fields[].kind` (`text|numeric|big_o|predict_output`, default `text`) on `parts[].config` and `revision.probes[].config`, in `curriculum/_schema/item.schema.json` and `internal/course/item.go`.
     ...
     - **Then set them on the gc content** (every multi/order part and probe; every predict-output field) and re-run the `content` job. Those gc hashes change on purpose; step 7 refreshes the pack and has the owner re-confirm the stamps.
   - **`Present()` reveal rule on M3's path** (`concluded=true`, set by the gateway from practice's state; M3-14/M3-09). Don't add a second path.
     ...
     - Unconcluded mock: aggregate only. Run and arena: `skipped(not_evaluated_here)`. The one exception is the arena D17 spoiler, only if the owner accepted it (step 2).
   ...
   - **Extend the denylist tests:** no `answer`, `accept`, `rationale`, `distractors` or per-question correctness in any response before conclusion, in Run, in an unconcluded mock, or in the arena (after a recorded reveal, key + rationale only, and only if the owner accepted the F11 spoiler).
...
2. **[X] Registries + quiz widget + Workspace-Quiz + Touch probes + arena quiz study mode (AB14).**
   ...
   - **Arena quiz study mode (AB14 F11):** a deliverable here, since M3-12 handed it over. `web/src/screens/Arena.tsx` renders the item's `choice`/`blank` parts in `edit` mode (nothing graded; key steps show `not_evaluated_here`), with the "Nothing here counts toward your course." banner and **Mark studied**.
     - **Owner kept "no key in the arena":** public content only, no spoiler control, denylist unchanged.
     - **Owner accepted the D17 spoiler:** the confirm posts M3-09's `POST /api/problems/{id}/arena/reveal` (records `arena_revealed_at`, no cap). Only after that record does the gateway ask judge's `Present()` for key + rationale, under a new `arena_revealed` visibility (never from the client), with no per-question marks. Add a dated amendment note to ADR-0029 §2 and a Decisions-log row.
     - Vitests for F11.
...
6. **[X] Merge + tag v1.15.0.**
   ...
   - After the tag, by looking: healthz, `get deploy`, ImagePolicy latest = tag, HelmReleases Ready, the smoke test (login, dashboard, coach), plus the Catalog `preview` badge, a gc item opening, and a DSA problem grading as before.

7. **[E · O] Evalpack `v1.N.0`** in `../xlearn-evalpack` (after v1.15.0 is live):
   - **Contract refresh:** `packlint check` names the gc items whose hash step 1 changed; add the new hashes to their `accepts_contract_hashes`. **The owner re-confirms those key stamps** (about 10 min). If the owner is away, this step waits ⛔ "pending owner".
...
8. **[O] Prod verify + owner run.**
   ...
   - Hand the owner the checklist: enroll, complete **one quiz item** and **one race item** (one failing submit first, if willing), then confirm. If the owner is away, set plan task 8 ⛔ "pending owner" and _Overall_ 🔄.
...
- **v1.15.0** tagged and verified; **evalpack `v1.N.0`** with the `gorace` CI job's gates, refreshed gc hashes, re-confirmed stamps, digest recorded; the owner's graded run.

## Update status

- In [`../sprints/sprint-p-03.md`](../sprints/sprint-p-03.md): set the task rows ✅ (or ⛔ with the reason), and set _Overall_ ✅ once the owner's run is done.
- In [`../status.md`](../status.md):
  - **Sprint board** P-03 ✅. **Milestones** P ✅ (runner-v1.1.0 → v1.15.0 → evalpack `v1.N.0`; exit: manifest + content, widget/profile code only, `preview`, owner graded one quiz and one race item);
...
- [ ] **The owner completed one quiz and one race item, graded**, on prod.
...
- [ ] Evalpack `v1.N.0`: the go-race gates passed in the `gorace` CI job (deadlines from P-01's prod baseline), changed stamps re-confirmed, anonymous GET 401/403, gc items `ok`, 0 `spec_mismatch`. v1.15.0 is live and verified; status.md is updated.

**Ship at session end** per AGENT.md land-and-sync with **this sprint's release action: tag v1.15.0 (the next free minor), then evalpack `v1.N.0`**. In order:
1. the xlearn PR → CI green → squash-merge → tag → Flux deploys → verify per the checklist;
2. the evalpack PR → CI green → squash-merge → tag → the IUA bumps judge → verify;
3. the owner's graded run (or ⛔ pending owner, recorded);
4. `git checkout main && git pull` in every repo touched (xlearn and `../xlearn-evalpack`).

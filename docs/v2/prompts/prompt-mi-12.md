...
- [ ] **Anthropic Console setup** (`ev-provider-runbook`, ~45 min; plan task 3, with the plan's task 2 table as the checklist):
  ...
  - WIF: service account `xlearn-judge` (member of `xlearn-platform-prod` only), the issuer and inline JWKS, and the federation rule as the plan's WIF row specifies (`check_jti` per spk-03). Read the issuer URL and the JWKS (public keys) live with `ssh vps 'sudo k3s kubectl get --raw /.well-known/openid-configuration'` and `… get --raw /openid/v1/jwks`;

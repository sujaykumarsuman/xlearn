...
|---|------|-------|----------|-----|
| D42 | 2026-09-26 | M6 voice shell | **GPT-Live-1 with the fixed design (owner), overriding the D28 outcome.** S6 (spk-04) failed both shells on a hard gate: GPT-Live on M2/M3/M3b (all traced to director pushes that prompt speech and to client delegation), mini on M2/M3/M12. The owner chose GPT-Live-1 on M14 5 vs 4, M12 0 wrong, M10 5 and the L25 variant (M3b and M12 pass, M2 from context at the edge), to be refined during the build. The fixed design: raw numbered code pushed only at the candidate's turn start, a director that feeds facts only, answers from context. A ≤ 1 h `ev-s6-recheck` confirms it before [ds-m6a-01](sprints/sprint-ds-m6a-01.md) accepts ADR-0032; if it fails, the owner re-decides (text-only per D28 stays the fallback). Details: [t6 §16.3](research/t6-realtime-interviewer.md#163-owner-decision-d42-the-fixed-design-and-the-re-check). | [0032](../adr/0032-realtime-ai-mock-interviewer.md) (Proposed until ds-m6a-01) |
| D41 | 2026-09-25 | Build plan | **Spikes first (owner).** All four spikes run before any build sprint so every design yes/no is answered first: spk-01 + spk-02 on Fri 2026-09-25 (agent-only), spk-03 + spk-04 on Sat 2026-09-26 (Console step; owner present). spk-02 uses the owner's spare `skriptvalley-vps` (free for PoCs) and a private registry instead of waiting for mi-07. S6 gets a ≤ 1 h non-blocking recheck before ds-m6a-01. S6 sole-passer tie-break: yes, a sole passer wins. The old `ssh vps` alias is now `sujaykumar-vps`. | [status.md](status.md) |
| D40 | 2026-09-25 | Build plan | **Sprint merge directive (owner).** The owner's words: "each prompt to finish with changes land-and-sync and consider my approval for all changes for the given prompt. Do not stop for my review." **Launching a v2 sprint prompt is the owner's approval for every change it makes**: merges, tags (contract, erase and GA ones included), infra PRs, board freezes, ADR acceptances, drafted owner content, and the production steps the prompt specifies. Every prompt ends with `## Ship (land-and-sync — owner approval pre-granted)`. **No review stops.** Design PRs merge on CI green, and the merge is the freeze (this supersedes D38's review gate; the owner may review afterwards). The GA PR merges on CI green. Spikes land only their results. Owner-only actions move to each prompt's `## Before you launch (owner)` block; if one is missing, the session lands what doesn't depend on it and marks the gap ⛔. The owner's review of the privacy-notice text moves to the v3 opening gates. | [AGENT.md](../../AGENT.md) · [status.md](status.md) |
...
Evidence came from code at `main`, `../infra`, read-only `ssh sujaykumar-vps` (get/describe/top, NATS `/varz` and `/jsz`, `sar`) and the web. The slices were synthesized into one draft, which faced an adversarial critique (7/10). All 6 majors were fixed; the 13 minors were routed to the ADRs and the rollout plan. The owner verified the live-security findings before deciding.

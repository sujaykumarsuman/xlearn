...
- **Last updated:** 2026-09-25 (spk-02 re-run block 2: Q-C GO, amd64 allowlists, x86_64-only proposal; block 1: image volume GO, GOCACHE seed; earlier: spk-02 partial results, ⛔ rows open; earlier the same day, D40: the sprint merge directive applied to the plan, status and indexes; before
...
| 17 | [spk-02](sprints/sprint-spk-02.md) | P3 amd64 replay + image-volume spike (throwaway) | MI | spike | no merge (spike, throwaway); results docs PR | 🔄 Re-run in progress (2026-09-25): **block 1 ✅** (image volume **GO**, with kubelet defaults; the GOCACHE seed). **Block 2 ✅** (Q-C GO: TSAN needs no ASLR policy; amd64 allowlists at 0 unexpected SIGSYS; x86_64-only pod profile proposed). Block 3 (AppArmor remount narrowing) is still ⛔. The first session was interrupted after the P3 jail setup and probes and found the **AppArmor `remount` issue** ([t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02)) |
...
| MI-10 | Spike week: P0–P3 + image volume (+ WIF) | [spk-01](sprints/sprint-spk-01.md), [spk-02](sprints/sprint-spk-02.md), [spk-03](sprints/sprint-spk-03.md) · `ev-spike-week` | ○ | ◐ | **part 1 (spk-01): P0–P2 GO** (Q-A GO, Q-B GO; mechanism go-sandbox `forkexec.Runner`, no userns). **Part 2 (spk-02): ⛔ incomplete.** P3 was interrupted (the jail setup and probes pass on amd64; TSAN and the allowlists were not run). The re-run's **block 1 gives image-volume GO** (kubelet defaults, no `AlwaysVerify`) and **block 2 gives Q-C GO** (TSAN needs no ASLR policy; amd64 allowlists at 0 unexpected SIGSYS). The M3 line stays unticked until block 3 lands the scoped AppArmor remount rule · [spike record](#spike-record) |
...
| P3 amd64 replay + image volume (**MI-10 verdict**) | [spk-02](sprints/sprint-spk-02.md) | Fri 2026-09-25 (D41), env A `skriptvalley-vps` | **⛔ partial:** P3 jail setup + probes pass on amd64; AppArmor `remount` finding. **Re-run block 1: image volume GO**, GOCACHE seed measured. **Block 2: Q-C GO** (TSAN with no ASLR policy, PG, allowlists at 0 unexpected SIGSYS, x86_64-only proposed). The AppArmor narrowing (block 3) is still ⛔ | [t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02) | ✅ 2026-09-25: env A restored (`k3s-uninstall.sh`; registry, images, host files, sysctls, packages and `/root/xl-spike` removed; apport and the kernel-meta holds restored; the landing container still serving). Kernel stays upgraded to 6.8.0-142 (approved). `xl-spike` purged. `~/xl-spike/` kept | [m3-03](sprints/sprint-m3-03.md), [mi-09](sprints/sprint-mi-09.md), [p-01](sprints/sprint-p-01.md) (TSAN) |
...
| [spk-02](sprints/sprint-spk-02.md) | [mi-09](sprints/sprint-mi-09.md) | ([t3 §16.2](research/t3-sandbox.md#162-p3-amd64-replay-spk-02)) **The broad AppArmor `remount,` rule must not ship.** Under it, read-write remounts of `/` and `/sys` succeeded from the supervisor context. Replace it with remount rules scoped to the jail tree, add both remount probes to the must-deny set, and re-prove the jail setup. **The amd64 pod seccomp file is final** (re-run block 2): §16.1's recipe on amd64 with `architectures: [SCMP_ARCH_X86_64]` (x86_64-only, proposed to m3-03). It has 381 names and `pivot_root`, and is recorded verbatim in t3 §16.2 (sha256 `730a7a55…418d`). If m3-03 keeps the three-arch baseline, only that line changes. | ⛔ AppArmor rule not finalised (block 3); ✅ seccomp file recorded |
| [spk-02](sprints/sprint-spk-02.md) | [m3-03](sprints/sprint-m3-03.md) | ([t3 §16.2](research/t3-sandbox.md#162-p3-amd64-replay-spk-02), §16.4) ADR-0030 deltas from block 2: the mechanism reproduces on amd64 6.8.0-142. **Pod profile: x86_64-only proposed.** Nothing legitimate broke, and the ia32 `int $0x80` entry point (open under three-arch) is closed. Caveat: runc's bad-arch action is `KILL_THREAD`. **ASLR policy: none** (TSAN never calls `personality`; `ADDR_NO_RANDOMIZE` stays denied). The allowlists' home is m3-04, with fixed rules: non-x86_64/x32 → KILL, `clone3` → ENOSYS, `clone` with `CLONE_THREAD` only, `prctl` with `PR_SET_VMA` only. The AppArmor rule follows in block 3. | ✅ recorded (block 2); AppArmor ⛔ block 3 |
...
| **Re-run spk-02's ⛔ rows** (~~TSAN, PG, amd64 allowlists + KILL, x86_64-only profile~~, scoped AppArmor remount rule, ~~image volume (i)–(iii)~~) | before mi-09's host-window PR and before M3 | [spk-02](sprints/sprint-spk-02.md) relaunched on env A `skriptvalley-vps` on 2026-09-25 (second session), in three blocks | 🔄 block 1 ✅ (image volume GO); block 2 ✅ (Q-C GO); block 3 in progress |
...
- ⛔ **MI-10 part 2 ([spk-02](sprints/sprint-spk-02.md)) is incomplete.** The first session was interrupted on 2026-09-25. The re-run's block 1 (image volume **GO**) and block 2 (Q-C GO: TSAN, PG, the amd64 allowlists, the KILL re-run, x86_64-only) landed. The scoped AppArmor remount rule (block 3) is still needed before [mi-09](sprints/sprint-mi-09.md)'s host-window PR and the M3 checklist ([t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02)).

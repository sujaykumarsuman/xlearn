...
- **Last updated:** 2026-09-25 (spk-02 partial results, ⛔ rows open; earlier the same day, D40: the sprint merge directive applied to the plan, status and indexes; before
...
| 17 | [spk-02](sprints/sprint-spk-02.md) | P3 amd64 replay + image-volume spike (throwaway) | MI | spike | no merge (spike, throwaway); results docs PR | ⛔ Partial (interrupted 2026-09-25): P3 jail setup + probes on amd64 recorded, plus the **AppArmor `remount` finding**; TSAN, allowlists and image volume not run, re-run needed ([t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02--partial-)) |
| 18 | [spk-03](sprints/sprint-spk-03.md) | WIF spike (≤ ½ day, throwaway) | MI | spike | no merge (spike, throwaway); results docs PR | ⬜ Not started |
...
| MI-10 | Spike week: P0–P3 + image volume (+ WIF) | [spk-01](sprints/sprint-spk-01.md), [spk-02](sprints/sprint-spk-02.md), [spk-03](sprints/sprint-spk-03.md) · `ev-spike-week` | ○ | ◐ | **part 1 (spk-01): P0–P2 GO** (Q-A GO, Q-B GO; mechanism go-sandbox `forkexec.Runner`, no userns). **Part 2 (spk-02): ⛔ incomplete.** P3 was interrupted (the jail setup and probes pass on amd64; TSAN and the allowlists were not run) and the image volume was not run. The M3 line stays unticked until the ⛔ rows are re-run · [spike record](#spike-record) |
...
| MI-14 | M4 gates: WIF spike, judge 443 egress, `xlearn-judge-llm` secret, provider runbook ($15 / $12) | [spk-03](sprints/sprint-spk-03.md) (WIF item) + [mi-12](sprints/sprint-mi-12.md) + `ev-provider-runbook` | ○ | ⬜ | WIF: ⬜ |
...
| [0031](../adr/0031-platform-ai-and-two-tier-keys.md) | platform AI and two-tier keys | Proposed | at the WIF spike, before M4: [mi-12](sprints/sprint-mi-12.md) (consumes [spk-03](sprints/sprint-spk-03.md)) |
...
| `ev-spike-goahead` | by Fri 2026-10-09 (booked together with ev-spike-week and ev-host-window) · before launch of spk-02 | Spike go-ahead (D23): **automatic since D40**, since launching [spk-01](sprints/sprint-spk-01.md) is the go-ahead for P0–P3 + image volume and launching [spk-03](sprints/sprint-spk-03.md) is the WIF yes. Owner-only part: the **second-VPS answer** for [spk-02](sprints/sprint-spk-02.md) (A: an empty amd64 VPS with SSH access and reimage approval; else B: the private GitHub Actions scratch repo) | none (owner) | ✅ spk-01 go-ahead exercised 2026-09-25 (launch = D23 go-ahead, D40); D41 says spk-02 runs on `skriptvalley-vps` (private registry, MI-9/chart-0.3.0 gates dropped) |
| `ev-spike-week` | **Fri 2026-09-25 → Sat 2026-09-26 (D41, spikes first)**; was Mon 2026-10-12 → Fri 2026-10-16 | Spike week: P0–P3 + image volume (+ WIF if it fits) | [spk-01](sprints/sprint-spk-01.md) | 🔄 in progress: spk-01 (P0–P2) done 2026-09-25; spk-02 partial ⛔ 2026-09-25 (its ⛔ rows need a re-run); spk-03 and spk-04 on Sat 2026-09-26 |
...
| P3 amd64 replay + image volume (**MI-10 verdict**) | [spk-02](sprints/sprint-spk-02.md) | Fri 2026-09-25 (D41), env A `skriptvalley-vps` | **⛔ partial:** P3 jail setup + probes pass on amd64; AppArmor `remount` finding; TSAN, PG, allowlists, KILL, x86_64-only and image volume **not run** | [t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02--partial-) | ✅ 2026-09-25: env A restored (`k3s-uninstall.sh`; registry, images, host files, sysctls, packages and `/root/xl-spike` removed; apport and the kernel-meta holds restored; the landing container still serving). Kernel stays upgraded to 6.8.0-142 (approved). `xl-spike` purged. `~/xl-spike/` kept | [m3-03](sprints/sprint-m3-03.md), [mi-09](sprints/sprint-mi-09.md), [p-01](sprints/sprint-p-01.md) (TSAN) |
| WIF (≤ ½ day) | [spk-03](sprints/sprint-spk-03.md) | Fri 2026-10-16 if it fits, else before M4 | ⬜ | — | ⬜ | [mi-12](sprints/sprint-mi-12.md) (ADR-0031) |
...
| [spk-02](sprints/sprint-spk-02.md) | [mi-09](sprints/sprint-mi-09.md) | ([t3 §16.2](research/t3-sandbox.md#162-p3-amd64-replay-spk-02--partial-)) **The broad AppArmor `remount,` rule must not ship.** Under it, read-write remounts of `/` and `/sys` succeeded from the supervisor context. Replace it with remount rules scoped to the jail tree, add both remount probes to the must-deny set, and re-prove the jail setup. The amd64 pod profile = §16.1's recipe on amd64 (381 names, `pivot_root` added; +`arch_prctl`/`modify_ldt` against arm64). Its verbatim JSON and the architecture choice wait for the re-run. | ⛔ recorded; rule not finalised |
| [spk-02](sprints/sprint-spk-02.md) | [m3-03](sprints/sprint-m3-03.md) | ADR-0030 inputs so far: the mechanism reproduces on amd64 6.8.0-142. The ia32 `int $0x80` entry point is open under the three-arch baseline, and x32 returns ENOSYS; the x86_64-only variant is not yet run. The RuntimeDefault-derived pod profile denies `personality(ADDR_NO_RANDOMIZE)`. | ⛔ partial |
| [spk-02](sprints/sprint-spk-02.md) | [p-01](sprints/sprint-p-01.md), [m3-04](sprints/sprint-m3-04.md) | TSAN under `mmap_rnd_bits=32` and the amd64 exec allowlists (go, cpp, python, go-race) were **not produced** ⛔. The in-pod overlay still EACCES on amd64. | ⛔ open (re-run) |
| [spk-02](sprints/sprint-spk-02.md) | [m3-07](sprints/sprint-m3-07.md), [m3-02](sprints/sprint-m3-02.md) | Image volume (i)–(iii) **not run** ⛔. Kubelet defaults recorded: `NeverVerifyPreloadedImages`, with `KubeletEnsureSecretPulledImages` beta and on. No fallback chosen; the pre-decided chain still applies. | ⛔ open (re-run) |
...
| **Re-run spk-02's ⛔ rows** (TSAN, PG, amd64 allowlists + KILL, x86_64-only profile, scoped AppArmor remount rule, image volume (i)–(iii)) | before mi-09's host-window PR and before M3 | relaunch [spk-02](sprints/sprint-spk-02.md) on env A `skriptvalley-vps` (still free for PoCs, D41; it was restored to baseline, so the setup is repeated). The MI-10 line stays ⛔ until then | ⬜ owner to relaunch |
...
| Date | Decision | Notes |
|------|----------|-------|
| 2026-09-25 | **spk-02: partial, interrupted ⛔.** On env A (`skriptvalley-vps`, amd64, kernel 6.8.0-142, `mmap_rnd_bits=32`, k3s v1.36.4) the §16.1 jail setup reproduces, and spk-01's 17 negative probes plus `move_mount`/`mount_setattr` stay denied under the amd64 pod profile. **Finding → [mi-09](sprints/sprint-mi-09.md):** the broad AppArmor `remount,` rule lets the supervisor remount `/` and `/sys` read-write, so it must be replaced by jail-scoped remount rules before the host window. The line was stopped at the finding, as the probe rules require. **Observed:** the ia32 `int $0x80` entry point is open under the three-arch baseline (evidence for x86_64-only, m3-03); the RuntimeDefault-derived pod profile denies `personality(ADDR_NO_RANDOMIZE)` (ASLR policy still open, p-01). **Not run:** TSAN, PG, the allowlists, the KILL re-run, the x86_64-only variant and image volume (i)–(iii). No fallback was chosen, the pre-decided chain still applies, and the MI-10 line stays ⛔. | [t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02--partial-). Teardown done the same day. No production change. |
...
- ⛔ **MI-10 part 2 ([spk-02](sprints/sprint-spk-02.md)) is incomplete.** The session was interrupted on 2026-09-25. TSAN, the amd64 allowlists, the x86_64-only profile, the scoped AppArmor remount rule and image volume (i)–(iii) need a re-run on env A before [mi-09](sprints/sprint-mi-09.md)'s host-window PR and the M3 checklist ([t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02--partial-)).

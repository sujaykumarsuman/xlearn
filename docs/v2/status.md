...
- **Last updated:** 2026-09-25 (spk-02 re-run block 1: image volume GO, GOCACHE seed; earlier: spk-02 partial results, ⛔ rows open; earlier the same day, D40: the sprint merge directive applied to the plan, status and indexes; before
...
| 17 | [spk-02](sprints/sprint-spk-02.md) | P3 amd64 replay + image-volume spike (throwaway) | MI | spike | no merge (spike, throwaway); results docs PR | 🔄 Re-run in progress (2026-09-25): **block 1 ✅**, with image volume **GO** (kubelet defaults) and the GOCACHE seed measured. Blocks 2–3 (TSAN, PG, allowlists, KILL, architectures, AppArmor remount narrowing) are still ⛔. The first session was interrupted after the P3 jail setup and probes and found the **AppArmor `remount` issue** ([t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02)) |
...
| MI-10 | Spike week: P0–P3 + image volume (+ WIF) | [spk-01](sprints/sprint-spk-01.md), [spk-02](sprints/sprint-spk-02.md), [spk-03](sprints/sprint-spk-03.md) · `ev-spike-week` | ○ | ◐ | **part 1 (spk-01): P0–P2 GO** (Q-A GO, Q-B GO; mechanism go-sandbox `forkexec.Runner`, no userns). **Part 2 (spk-02): ⛔ incomplete.** P3 was interrupted (the jail setup and probes pass on amd64; TSAN and the allowlists were not run). The re-run's **block 1 gives image-volume GO** (kubelet defaults, no `AlwaysVerify`). The M3 line stays unticked until blocks 2–3 land · [spike record](#spike-record) |
...
| P3 amd64 replay + image volume (**MI-10 verdict**) | [spk-02](sprints/sprint-spk-02.md) | Fri 2026-09-25 (D41), env A `skriptvalley-vps` | **⛔ partial:** P3 jail setup + probes pass on amd64; AppArmor `remount` finding. **Re-run block 1: image volume GO**, GOCACHE seed measured. TSAN, PG, allowlists, KILL, x86_64-only and the AppArmor narrowing are still ⛔ (blocks 2–3) | [t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02) | ✅ 2026-09-25: env A restored (`k3s-uninstall.sh`; registry, images, host files, sysctls, packages and `/root/xl-spike` removed; apport and the kernel-meta holds restored; the landing container still serving). Kernel stays upgraded to 6.8.0-142 (approved). `xl-spike` purged. `~/xl-spike/` kept | [m3-03](sprints/sprint-m3-03.md), [mi-09](sprints/sprint-mi-09.md), [p-01](sprints/sprint-p-01.md) (TSAN) |
...
| [spk-02](sprints/sprint-spk-02.md) | [p-01](sprints/sprint-p-01.md), [m3-04](sprints/sprint-m3-04.md) | TSAN under `mmap_rnd_bits=32` and the amd64 exec allowlists (go, cpp, python, go-race) were **not produced** ⛔ (re-run block 2). The in-pod overlay still EACCES on amd64; the GOCACHE replacement is in the row above. | ⛔ open (re-run block 2) |
...
| **Re-run spk-02's ⛔ rows** (TSAN, PG, amd64 allowlists + KILL, x86_64-only profile, scoped AppArmor remount rule, ~~image volume (i)–(iii)~~) | before mi-09's host-window PR and before M3 | [spk-02](sprints/sprint-spk-02.md) relaunched on env A `skriptvalley-vps` on 2026-09-25 (second session), in three blocks | 🔄 block 1 ✅ (image volume GO); blocks 2–3 in progress |
...
- ⛔ **MI-10 part 2 ([spk-02](sprints/sprint-spk-02.md)) is incomplete.** The first session was interrupted on 2026-09-25. The re-run's block 1 landed (image volume **GO**). TSAN, PG, the amd64 allowlists, the KILL re-run, the x86_64-only profile and the scoped AppArmor remount rule (blocks 2–3) are still needed on env A before [mi-09](sprints/sprint-mi-09.md)'s host-window PR and the M3 checklist ([t3 §16.2–16.4](research/t3-sandbox.md#162-p3-amd64-replay-spk-02)).

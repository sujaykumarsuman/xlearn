...
- **Build line:** v2 · **Phase:** **spikes first (D41)**, after the build-plan scaffold of 2026-09-24 (89 sprints: 86 plan +
  ...
  (WIF GO); 2026-09-26: [spk-04](sprints/sprint-spk-04.md) (S6: both fail under D28 → owner chose GPT-Live-1 with the
  fixed design, D42; [t6 §16](research/t6-realtime-interviewer.md#16-s6-results-spk-04-2026-09-26)). **Next: week 1:** [mi-01](sprints/sprint-mi-01.md) (first), [ds-m1-01](sprints/sprint-ds-m1-01.md),
...
| 71 | [spk-04](sprints/sprint-spk-04.md) | S6 voice-shell bake-off (owner present, throwaway) | M6a | spike | no merge (spike, throwaway); results docs PR | ✅ Done 2026-09-26 (results: [t6 §16](research/t6-realtime-interviewer.md#16-s6-results-spk-04-2026-09-26); both shells failed a hard gate under D28 → owner chose **GPT-Live-1 with the fixed design (D42)**, confirmed at `ev-s6-recheck`) |
| 72 | [ds-m6a-01](sprints/sprint-ds-m6a-01.md) | Design M6a part 1 + ADR-0032 → Accepted: Mock-v2, setup/consent/pre-flight, live HUD text (AB13, AB24, AB25) | M6a | design | ADR-0032 docs PR, then land-and-sync; the merge is the design freeze | ⬜ Not started · gate: `ev-s6-recheck` must confirm D42 (GPT-Live-1, [fixed design](research/t6-realtime-interviewer.md#163-owner-decision-d42-the-fixed-design-and-the-re-check)) before ADR-0032 is accepted |
...
| 80 | [ds-m6b-01](sprints/sprint-ds-m6b-01.md) | Design M6b: voice pre-flight/notices, voice live HUD ★ (AB29, AB30 ★) | M6b | design | land-and-sync; the merge is the design freeze | ⬜ Not started · voice shell GPT-Live-1 per D42, gated by `ev-s6-recheck` |
| 81 | [mi-13](sprints/sprint-mi-13.md) | M6b gates: Permissions-Policy, camera/mic deny, coach sizing, SDP route, WSS egress (MI-16) → v2.0.x patch | MI | infra | **tag v2.0.x patch** (MI-16) + infra PRs first | ⬜ Not started · voice shell GPT-Live-1 per D42, gated by `ev-s6-recheck` |
| 82 | [m6b-01](sprints/sprint-m6b-01.md) | VoiceShell adapter + SDP broker + sideband | M6b | product | merge only (ships dark in m6b-03's patch) | ⬜ Not started · voice shell GPT-Live-1 per D42, gated by `ev-s6-recheck` |
| 83 | [m6b-02](sprints/sprint-m6b-02.md) | Voice robustness: lease, re-attach, drain, cost + $ cap, PTT, rollover (+ coach-interview if M7 failed) | M6b | product | merge only (ships dark in m6b-03's patch) | ⬜ Not started · voice shell GPT-Live-1 per D42, gated by `ev-s6-recheck` |
| 84 | [m6b-03](sprints/sprint-m6b-03.md) | Voice UI: pre-flight/notices (AB29), live HUD★ (AB30), consent, EU gate, self-view → v2.0.x patch (M6b dark) | M6b | product | **tag v2.0.x patch** (M6b dark, cohort) | ⬜ Not started · voice shell GPT-Live-1 per D42, gated by `ev-s6-recheck` |
| 85 | [m6b-04](sprints/sprint-m6b-04.md) | Fake-media e2e + interviewer GA flip → v2.1.0 | M6b | product | **tag v2.1.0** (interviewer GA) | ⬜ Not started · voice shell GPT-Live-1 per D42, gated by `ev-s6-recheck` |
...
| [0032](../adr/0032-realtime-ai-mock-interviewer.md) | realtime AI mock interviewer | Proposed | at S6, before the M6a design freeze: [ds-m6a-01](sprints/sprint-ds-m6a-01.md) task 1. **S6 reported 2026-09-26: both shells failed a hard gate under D28; the owner chose GPT-Live-1 with the fixed design (D42)**; acceptance waits for `ev-s6-recheck` and folds in the amendments in [t6 §16.8](research/t6-realtime-interviewer.md#168-for-ds-m6a-01--ab24-and-ds-m6b-01--ab29) |
...
| `ev-spike-week` | **Fri 2026-09-25 → Sat 2026-09-26 (D41, spikes first)**; was Mon 2026-10-12 → Fri 2026-10-16 | Spike week: P0–P3 + image volume (+ WIF if it fits) | [spk-01](sprints/sprint-spk-01.md) | ✅ done: spk-01 (P0–P2) 2026-09-25; spk-02 2026-09-25 (P0–P3 GO, image volume GO → MI-10 ✅); spk-03 2026-09-25 (WIF GO, `check_jti=false`); spk-04 2026-09-26 (S6: both shells fail a hard gate → ⛔ owner decision) |
...
| `ev-s6` | **Sat 2026-09-26, owner present (D41)** | S6 voice-shell bake-off (≤ 1 day, $10 hard limit): the owner creates spk-04's two throwaway OpenAI projects and keys before launching it, and stays present for the day | [spk-04](sprints/sprint-spk-04.md) | ✅ 2026-09-26: owner present for the day (S6 ran ~10:30–15:00 IST). Projects `xlearn-s6` ($10 hard limit; spent $6.50) and `xlearn-s6-quota` ($1 → $3 in one step; spent $2.41); org limit raised to $12. Keys revoked and both projects archived by the owner the same day. Results: [t6 §16](research/t6-realtime-interviewer.md#16-s6-results-spk-04-2026-09-26) |
| `ev-s6-recheck` | before launch of ds-m6a-01 (≈ Q1 2027) | ≤ 1 h S6 re-check **of D42's fixed design** (GPT-Live only): M2 (perceived, n ≥ 8), M3 (≥ 6 min of marked windows), M3b, M12, M4 (marked), M14; plus a note if the model, price or docs changed. **Blocking for ds-m6a-01's ADR-0032 acceptance (D42)**; still non-blocking for everything before M6a | [spk-04](sprints/sprint-spk-04.md) | ⬜ |
...
| S6 voice-shell bake-off (≤ 1 day, $10 limit) | [spk-04](sprints/sprint-spk-04.md) | Sat 2026-09-26 (D41), owner present; localhost harness on the owner's Mac (Chrome 153, Firefox 156, Safari 26.5; home Wi-Fi, India) | **Both fail under D28; the owner then chose GPT-Live-1 with the fixed design (D42), confirmed at `ev-s6-recheck`.** GPT-Live-1 fails M2/M3/M3b (all caused by director context pushes + client delegation), passes M5/M6/M7/M9/M12/M13, M14 5; mini fails M2/M3/M12, passes M3b/M5/M6/M7/M9/M13, M14 4. **M7 passes on both → one coach Deployment.** Browsers: Chrome/Edge + Safari; Firefox → text. An exploratory turn-boundary-push re-test of GPT-Live (L25) is recorded as evidence | [t6 §16](research/t6-realtime-interviewer.md#16-s6-results-spk-04-2026-09-26) · [fixtures](research/t6-s6-fixtures/) | ✅ 2026-09-26: keys revoked, both projects archived (owner); harness dir, raw logs, WAVs, Chrome profiles deleted; `~/.zsh_history` holds 2 key-shaped lines → [Open owner items](#open-owner-items) | [ds-m6a-01](sprints/sprint-ds-m6a-01.md) (ADR-0032 after `ev-s6-recheck`), [m6a-02](sprints/sprint-m6a-02.md) (fixtures), [mi-13](sprints/sprint-mi-13.md), [m6b-01](sprints/sprint-m6b-01.md)/[m6b-02](sprints/sprint-m6b-02.md) |
...
| S6 scheduling | before the M6a design freeze (before launch of ds-m6a-01) | **Sat 2026-09-26 (D41)**, owner present; ≤ 1 h `ev-s6-recheck` before ds-m6a-01 | ✅ ran 2026-09-26 |
| **S6 decision: what v2.1.0 carries** | before launch of [ds-m6a-01](sprints/sprint-ds-m6a-01.md) (ADR-0032 acceptance) | **D42 (owner, 2026-09-26): GPT-Live-1 with the fixed design**, confirmed by `ev-s6-recheck` (≤ 1 h, owner present) before ds-m6a-01; [spec](research/t6-realtime-interviewer.md#163-owner-decision-d42-the-fixed-design-and-the-re-check) | ✅ decided · re-check ⬜ |
| **Inspect `~/.zsh_history`** | soon | spk-04's count-only key check found **2 key-shaped lines** (pattern `KEYRE`; never printed; they may be unrelated keys). Inspect and clean them; revoke any live key they expose | ⬜ open |
| S6 step 10: passive prepaid timing | your next real OpenAI top-up | time purchase → probe success on your normal key and append it to t6 §16.10 | ⬜ open |
| Optional S6 tidy-up | any time | set the org spend limit back to $10 (raised to $12 for S6); delete the two 1Password items (keys already revoked) | ⬜ optional |
...
- **`ev-s6-recheck` (D42):** a ≤ 1 h GPT-Live re-check of the fixed design, with the owner present, now gates [ds-m6a-01](sprints/sprint-ds-m6a-01.md)'s ADR-0032 acceptance and so the M6b sprints. It does not gate the v2.0 build: per D41, week 1 ([mi-01](sprints/sprint-mi-01.md) first) can start now that all four spikes have landed.

...
- **Count cross-check.** `ssh sujaykumar-vps 'k3s kubectl exec -n xlearn deploy/xlearn-judge -- judge admin status --json'` ([m3-14](sprint-m3-14.md) task 5; there is **no `--path` flag**, and `status` reports only evaluable **counts** by course and status `ok|spec_mismatch|unsupported|invalid`, with no item ids and no `no_pack` count). This is the sanctioned admin-CLI path ([ADR-0033](../../adr/0033-invite-only-admission-and-owner-admin.md), D33). It is not side-effect free: **every `judge admin` verb, reads included, writes an `admin_audit` row**, so the script calls it once per run.
- **Assertions:**

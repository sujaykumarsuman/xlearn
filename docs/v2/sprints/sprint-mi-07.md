...
> **Prereqs:** none. MI-9 depends on no other MI step ([rollout §2](../rollout-plan.md)); only the owner's machine-user action (calendar event `ev-machine-user`).
...
> **Release action:** **infra PR(s) only**, plus the new evalpack repo scaffold. **Only a `v0.1.0` image; no `>=1.0.0` tag.** Plus an xlearn docs PR for status.
> **Calendar:** weeks 1–2. **Must land by Fri 2026-10-09**: spk-02 runs the image-volume spike on Thu 2026-10-15, and pack authoring starts at the ≈ 2026-10-05 schema freeze.
> **Execute with:** [`../prompts/prompt-mi-07.md`](../prompts/prompt-mi-07.md) — one prompt, one session.
...
| # | Task | Repo | Status |
|---|------|------|--------|
| 1 | Machine user + classic `read:packages` PAT | O | ⬜ |
...
| 4 | Grant the machine user read on the package; verify a PAT pull | O | ⬜ |
...
- [ ] None from the MI track: MI-9 depends on no other MI step ([rollout §2](../rollout-plan.md)). The only prerequisite is the owner's machine-user action (task 1), which can run in parallel with task 2.
- [ ] `sujaykumarsuman/xlearn-evalpack` doesn't exist yet (`gh repo view` → not found; true on 2026-09-24), and no peer session is creating it (ListAgents, `gh repo list`). If it exists, stop and verify it is private, not a fork or template of xlearn, and that its package isn't public, before reusing it.
...
**In**
- MI-9:
  - the machine user and a classic `read:packages` PAT (owner);
...
The owner, as calendar event `ev-machine-user` (about 20 minutes):
...
- Sets an expiry of at most 1 year, and gives the **expiry date** to the agent. The PAT value itself never goes in chat, a terminal argument or git.

Creating accounts is owner-only.
...
- In the package settings, confirm the visibility is **Private** and the package is linked to `xlearn-evalpack`.
...
- **The helper:** a new `../infra/hack/evalpack-pull-secret.sh`, which the **owner runs** at creation and at every rotation. It:
...
- **Ship it in the same infra PR as task 5,** so the Secret exists when the ImageRepository first reconciles. After the merge (read-only, `ssh vps`):
...
- **MI track:** MI-9 ✅ (evalpack repo created date, CI run link, infra PR). Note: *ImagePolicy deferred to m3-07 (image before policy)*.
...
Also update this file's Status table. Ship it as an xlearn docs PR.
...
**infra PR(s) only**, plus the evalpack repo scaffold:
- One `../infra` PR carries the two SOPS pull secrets, `hack/evalpack-pull-secret.sh`, the ImageRepository, the README rotation section and the host-lint change.
...
- `0.1.0` is pushed, private, and pullable with the PAT.
- The infra PR is merged, with the ImageRepository Ready and no ImagePolicy.
- Statuses and the expiry row are updated (this file + [`../status.md`](../status.md)), and the xlearn docs PR is merged.

...
_Overall:_ 🔄 **Re-run in progress (2026-09-25, second session).** The first session was interrupted after the P3 jail setup and probes. The re-run lands in three blocks: **block 1 ✅** (image volume **GO**, and the GOCACHE seed); **block 2 ✅** (Q-C GO: TSAN, PG, the allowlists, KILL, x86_64-only); block 3 (AppArmor remount narrowing) is still ⛔. See [t3 §16.2–16.4](../research/t3-sandbox.md#162-p3-amd64-replay-spk-02).

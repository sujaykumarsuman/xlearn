...
_Overall:_ 🔄 **Re-run in progress (2026-09-25, second session).** The first session was interrupted after the P3 jail setup and probes. The re-run lands in three blocks: **block 1 ✅** (image volume **GO**, and the GOCACHE seed); block 2 (TSAN, PG, allowlists, KILL, architectures) and block 3 (AppArmor remount narrowing) are still ⛔. See [t3 §16.2–16.4](../research/t3-sandbox.md#162-p3-amd64-replay-spk-02).
...
| 2 | P3 replay: the amd64 pod seccomp profile, TSAN under `mmap_rnd_bits=32`, amd64 allowlists (Go, C++, Python), KILL re-run (2 h) | H | ⛔ partial (interrupted). The profile is generated, and the jail setup and probes pass on amd64. **Finding: the broad AppArmor `remount,` rule allows read-write remounts of `/` and `/sys`.** TSAN, PG, the allowlists, the KILL re-run and the x86_64-only variant were not run ([t3 §16.2](../research/t3-sandbox.md#162-p3-amd64-replay-spk-02)). Re-run block 1: the **GOCACHE seed without an overlay ✅** (a read-only seed used in place takes a compile from ~12 s to ~0.35 s); blocks 2–3 pending |

...
> **Prereqs:** [l-01](sprint-l-01.md) (v1.11.0: all 4 consumers bound) · [ds-l-01](sprint-ds-l-01.md) (AB21 frozen) · [mi-04](sprint-mi-04.md) (MI-5b live before the first tester) · [m1-04](sprint-m1-04.md) (`identity admin` CLI, session-validate `role`/`created_at`, `admin_audit`)
> **Unblocks:** [l-03](sprint-l-03.md) (L-A backend extends this erase transaction)
> **Release action:** **tag v1.12.0 — an erase tag: `host-verify --cluster` green and a manual snapshot first** (indicative: the next free minor). **The xlearn PR stays unmerged (CI-green) until the snapshot is taken; then merge and tag back to back**, so the erase producer never sits on `main` untagged for a peer's tag to ship. No infra PR.
> **Calendar:** early November, right after v1.11.0. **Owner events:** `ev-first-tester` (mint a tester via the CLI, only once MI-5b is live — prepared by [m1-04](sprint-m1-04.md)) and `ev-snap-v1.12.0` (the manual Hostinger snapshot right before the tag).
...
| 6 | Runbook (erase + R-d list) · compose rehearsal at the PR head · `host-verify --cluster` · owner snapshot `ev-snap-v1.12.0` (PR held unmerged until then) | X + H + O | ⬜ |
| 7 | Squash-merge + tag v1.12.0 back to back (erase tag; release checklist) | X | ⬜ |
| 8 | Post-tag acceptance on prod: a tester erases on the web | O + H | ⬜ |
...
- [ ] **AB21 frozen** ([ds-l-01](sprint-ds-l-01.md)): its PR merged by the owner. (AB19–AB20 froze in the same merge; this sprint records all three.)
...
- [ ] **MI-5b live** ([mi-04](sprint-mi-04.md)) **before the first tester is minted**: no admin console answers on `projects.sujaykumar.dev` (ADR-0033 §11).
- [ ] **First tester minted via the CLI after MI-5b** (`ev-first-tester`; `identity admin account create --role tester --email …`). Needed by task 8 only — tasks 1–7 may start before it.
...
- Cut **v1.12.0 as an erase tag**: `host-verify --cluster` green and a manual snapshot right before it
...
### 6 · Runbook, `host-verify`, snapshot [X + H + O]
...
- **The merge is held.** The xlearn PR is opened and brought to CI-green, but **not merged** until the snapshot exists: `deploy.yml`
  builds only on `v*` tags, so a merged-but-untagged producer on `main` would ship with **whichever tag comes next** — a parallel
  M3 sprint's (e.g. [m3-07](sprint-m3-07.md)'s v1.13.0, whose merge-only predecessors run beside L) or a v1 patch — with no
  `host-verify`, no snapshot and possibly out of version order, breaking [ADR-0034 §4.3/§6](../../adr/0034-v2-release-labelling-gating-and-rollback.md#43-snapshot-rule) and the D12 guard.
...
- **[O] `ev-snap-v1.12.0`** — the owner takes the manual Hostinger snapshot and confirms; record its id/time. **Then, at once:**
  re-check peers, squash-merge the PR (it must still be the rehearsed head; if `main` moved, rebase, re-run CI and the rehearsal first,
  and take a fresh snapshot if the old one is no longer "right before"), and tag the merge commit (task 7).
- **If the session has no owner at hand:** prepare everything, leave the PR **open, CI-green and unmerged**, record in status.md
  "l-02 PR #… green, **held for `ev-snap-v1.12.0`** — erase producer; do not merge without the snapshot", and hand the owner the exact
  steps (snapshot → merge → tag). The erase-tag rule overrides AGENT.md's "no open PRs at session end" for this one PR; the sprint
  stays 🔄. Never merge the producer without the snapshot being ready to follow it.

### 7 · Tag v1.12.0 [X]

The **compose rehearsal** runs at the PR head before the snapshot (task 6: `docker compose up`; create a tester through the CLI; seed an
...
profile 404, the username unavailable). Right after `ev-snap-v1.12.0`: squash-merge, then the **release checklist** (below; erase-tag
lines apply) and the tag on the merge commit, back to back. GitHub release title **`v1.12.0 — v2 build · L-E erase`**.
...
### 8 · Post-tag acceptance on prod [O + H]

With the tester from `ev-first-tester` (the owner operates it in a private window):
...
3. Checks, through the sanctioned admin CLI and read-only views only (no raw `psql` against other services' schemas):
   ...
   unavailable; no ERROR on the erase paths (`k3s kubectl logs -n xlearn deploy/xlearn-<svc> --since=30m` for practice, review,
   assessment, coach and identity, read-only).
4. The **owner-refused** path is verified by tests and by the owner's Settings showing F7 (no button) — **never send `DELETE /api/me`
   from the owner's account on prod**.
...
testers only; owner refused (CLI only)"; artboard rows AB19, AB20, AB21 → "frozen (PR #, date)" and [ds-l-01](sprint-ds-l-01.md) task 7 ✅
+ _Overall_ ✅; the erase log (request id, date, via, acks); the manual-path log; Decisions-log lines. Note [l-03](sprint-l-03.md) is unblocked.
...
- [ ] The PR stayed unmerged until the snapshot; `host-verify --cluster` green and **the snapshot id recorded** before the merge and the tag (back to back); v1.12.0 live and verified; status.md updated.
...
For this tag:
- **Erase line applies:** `host-verify --cluster` green, host settled, **snapshot taken** (`ev-snap-v1.12.0`) right before the squash-merge and the tag, which follow each other at once.
...
- Flags: none added (the web-erase allow-list is code). Also smoke the Settings erase card's owner state (F7).
...
green and the snapshot taken right before the merge + tag (never merged without it) · v1.12.0 deployed by Flux and verified by looking · the tester acceptance run on prod · acceptance criteria met ·
statuses updated (this file, [ds-l-01](sprint-ds-l-01.md) task 7, [`../status.md`](../status.md)) · local `main` synced.
...
- **The first tester needs MI-5b** (ADR-0033 §11). If MI-5b slips, tasks 1–7 can merge, but task 8 (and the tag's acceptance) waits.

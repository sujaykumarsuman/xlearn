...
- **Build line:** v1 · **Phase:** S06 **review** service (the **first async consumer**) + the **Revision ★** queue **built & green; shipping via Flux**. The five-touch spaced-repetition scheduler: durable pull consumers on `XLEARN_PRACTICE` schedule the Day 1/3/7/21/45 ladder on a first clean solve (+ the R-PF2 owed 3-day re-solve on an early reveal), auto-score each re-solve (pass advances · miss resets to Day 1 · Day 21/45 mock), and a ~15-min sweep materialises "due today" offline-safe. Emits `revision_scheduled`/`revision_due` via a **transactional outbox** to **NATS JetStream** (`XLEARN_REVIEW`); every handler dedupes on `event_id` via an **inbox** ([ADR-0015](../adr/0015-five-touch-scheduler-model.md)). New infra: `xlearn_review` DB role/schema + the `xlearn-review` HelmRelease (ClusterIP, `route.enabled:false`, `NATS_URL`) + gateway `REVIEW_BASE_URL`/`JWT_AUD_REVIEW`. Prior: S05 **practice** + **Problem ★** live (`0.1.15`). → No milestone (mid-phase); unblocks S07/S09.
- **Deploy mode:** build-semver auto-deploy from `main` ([ADR-0009](../adr/0009-deployment-and-gitops.md)). S05 shipped in **both** repos to `0.1.15`. Full authed end-to-end run still pending real GitHub OAuth creds (same ceiling as M1/M2).
...
| Application code | 🔄 S01 gateway + web shell live; S02 identity **merged & deployed** (`0.1.7`); S03 `curriculum` + Catalog/Roadmap **merged & deployed** (`0.1.11`); S04 Week + Concept + week-aggregation `agg` **merged & deployed** (`0.1.13`); S05 `practice` + Problem ★ **merged & deployed** (`0.1.15`); S06 `review` service (schema `review`, goose/sqlc, five-touch scheduler, auto-score, ~15m sweep, `internal/platform/events` **durable pull `NatsConsumer`** + outbox relay to `XLEARN_REVIEW`, idempotent inbox) + gateway Revision `agg`/proxy (`/revision/due` + `/revision/{id}/score`) + the **Revision ★** screen **built & green; shipping** |
| `infra` xlearn wiring | 🔄 gateway + identity + curriculum + practice live (`0.1.15`); S06 adds: `xlearn-review` HelmRelease (ClusterIP, `route.enabled:false`, `NATS_URL`); `review` schema + `xlearn_review` role (4-step) + SOPS `xlearn-review-db`/`pg-xlearn-review`; `xlearn-review` image-automation (ImagePolicy `>=0.1.0`); gateway `REVIEW_BASE_URL`/`JWT_AUD_REVIEW` |
...
| [S06](sprints/sprint-06.md) | review scheduler / Revision | 🔄 Built & green (real-PG integration + Docker image); shipping via Flux |

...
- **Build line:** v1 · **Phase:** S05 **practice** service (the first per-user, event-producing service) + the hero **Problem ★** workspace **merged & deployed to prod via Flux** (`xlearn-practice`/`xlearn-gateway`:`0.1.15`). Guided gated flow (attempt 15m → hint 10m → solution → re-implement → log) with **server-authoritative timers**, the reveal penalty, and outcome logging; emits `problem_solved`/`attempt_logged`/`solution_revealed_early` via a **transactional outbox** to **NATS JetStream** (`XLEARN_PRACTICE`). New infra: NATS in its own `messaging` namespace + the `xlearn_practice` DB role/schema + the `xlearn-practice` HelmRelease ([ADR-0014](../adr/0014-nats-jetstream-topology-and-outbox-relay.md)). Verified live: gateway `0.1.15`, `/api/problems/{id}` (`agg`) + attempt/reveal/outcome present and session-gated (→401). → **M3 reached.**
- **Deploy mode:** build-semver auto-deploy from `main` ([ADR-0009](../adr/0009-deployment-and-gitops.md)). S05 shipped in **both** repos (this + `../infra`): the shared `internal/platform/**` change rebuilt all four images to `0.1.15`; infra image-automation bumped the tags. Full authed end-to-end run still pending real GitHub OAuth creds (same ceiling as M1/M2).

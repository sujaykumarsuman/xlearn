...
| Application code | 🔄 S01 gateway + web shell live; S02 identity **merged & deployed** (`0.1.7`); S03 `curriculum` + Catalog/Roadmap **merged & deployed** (`0.1.11`); S04 Week + Concept + week-aggregation `agg` **merged & deployed** (`0.1.13`); S05 `practice` + Problem ★ **merged & deployed** (`0.1.15`); S06 `review` service (schema `review`, goose/sqlc, five-touch scheduler, auto-score, ~15m sweep, `internal/platform/events` **durable pull `NatsConsumer`** + outbox relay to `XLEARN_REVIEW`, idempotent inbox) + gateway Revision `agg`/proxy (`/revision/due` + `/revision/{id}/score`) + the **Revision ★** screen **merged & deployed** (`0.1.17`) |
| `infra` xlearn wiring | 🔄 gateway + identity + curriculum + practice + **review live** (`0.1.17`); S06 added: `xlearn-review` HelmRelease (ClusterIP, `route.enabled:false`, `NATS_URL`); `review` schema + `xlearn_review` role (4-step) + SOPS `xlearn-review-db`/`pg-xlearn-review`; `xlearn-review` image-automation (ImagePolicy `>=0.1.0`); gateway `REVIEW_BASE_URL`/`JWT_AUD_REVIEW` |

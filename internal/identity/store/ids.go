...
// (the account row's own id is generated in Postgres via gen_random_uuid()).
func newUUIDv4() string {
	var b [16]byte
	// crypto/rand.Read never returns a short read and only errors on a broken
	// platform RNG; treat that as fatal-to-the-caller via a zero uuid would be
	// wrong, so panic is avoided by letting the (impossible) error surface as a
	// still-well-formed, all-zero-derived value is not acceptable — instead we
	// rely on rand.Read's contract and ignore the error per stdlib guidance.
	_, _ = rand.Read(b[:])
